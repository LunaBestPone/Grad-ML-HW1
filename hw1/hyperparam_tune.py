...
def main():
    ...
    val_set_num_feature_matrix = val_set_num_feature_matrix.astype(np.float)
    ...
    for k in range(1,max_k+1):
        for i in range(val_set_label_matrix.shape[0]):
            ...
            if feature_status == [1,0]:
                d = np.sum(np.absolute(normed_training_set_num_feature_matrix - normed_val_set_num_feature_matrix[i]), 1)
            if feature_status == [0,1]:
                d = np.sum(val_set_vote_feature_matrix[i] != training_set_vote_feature_matrix, 1)
            if feature_status == [1,1]:
                d = np.sum(np.absolute(normed_training_set_num_feature_matrix - normed_val_set_num_feature_matrix[i]), 1) + np.sum(val_set_vote_feature_matrix[i] != training_set_vote_feature_matrix, 1)
